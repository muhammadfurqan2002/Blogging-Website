...
function DashPost() {
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setUserPost((post) => [...post, ...data.posts])
        if (data.posts.length < 9) {
          setShowMore(false)
        }
      }
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setUserPost(
          (prev) =>
            prev.filter((filterPost) => filterPost._id !== postId)
        )
      }
      ...
      <Modal show={showModal} onClose={() => setShowModal(false)} popup size='md' >
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <h3 className='mb-5 text-lg text-grey-500 dark: text-gray-400'>Are yu sure you want to delete post</h3>
